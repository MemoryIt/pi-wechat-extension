...
**问题描述**：
当用户发送纯图片时，如果 pi 正在处理上一条消息（AI 正在推理中），纯图片会被立即拦截并发送"图片已收到，成功保存到..."给微信。

**影响**：
用户无法收到正在处理中的那条消息的回复。

**根因**：
微信官方限制：新消息回复后，无法再次回复旧消息。

**复现步骤**：
1. 发送文本消息 A（触发 AI 推理）
2. 立即发送纯图片（此时 AI 还在推理中）
3. 微信收到"图片已收到..."消息
4. 文本消息 A 的 AI 回复丢失（无法再次发送）

**可能的解决方案**：
1. **检测 AI 是否空闲**：如果 `isAiProcessing=true`，将纯图片消息加入队列，等 AI 空闲后再处理
2. **延迟处理**：检测到纯图片时，先加入队列，不立即发送回复
3. **消息合并**：纯图片和后续追问合并处理
